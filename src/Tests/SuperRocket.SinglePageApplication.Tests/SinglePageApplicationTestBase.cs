...
{
    public abstract class SinglePageApplicationTestBase : AbpIntegratedTestBase<SinglePageApplicationTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected SinglePageApplicationTestBase()
        {
        ...
        protected void UsingDbContext(Action<SinglePageApplicationDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<SinglePageApplicationDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<SinglePageApplicationDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<SinglePageApplicationDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<SinglePageApplicationDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<SinglePageApplicationDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<SinglePageApplicationDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<SinglePageApplicationDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<SinglePageApplicationDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<SinglePageApplicationDbContext, Task<T>> func)
        {
            T result;
